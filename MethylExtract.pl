...
my %optionsDefault = (
	seq => "NA", inDir => "NA",
	outDir => "NA", qscore => "phred33-quals", p => 4,
	minQ => 20, methNonCpGs => 0.9, varFraction => 0.1,
	context => "CG", maxPval => 0.05, minDepthMeth => 3, maxStrandBias => 0.7,
 	bedOut => "N", wigOut => "N", delDup => "N",
 	FirstIgnor => 1000, LastIgnor => 1000,
 	minDepthSNV => 1,
 	flagW => "NA", flagC => "NA", 
 	tagW => "NA", tagC => "NA", #(deprecated input parameters)
 	simDupPb => 26, chromDiv => 400, memNumReads => 200000, 
 	chromSplitted => "N", 
...
my ($inDir,$seqDir,$context,$checkBisulfite,$methNonCpGs,$qscore,$Q,$minSNVperc,$outDir,$p,$delDup,$FirstIgnor,$tagW,$tagC,$simDupPb,$chromDiv,$memNumReads,$chromSort,$minDepthSNV,$maxPval,$minDepthMeth,$maxStrandBias,$typeFasta,$multiSeqID,$peOverlap,$LastIgnor);
...
($inDir,$seqDir,$context,$checkBisulfite,$methNonCpGs,$qscore,$Q,$minSNVperc,$outDir,$bedOut,$wigOut,$p,$delDup,$FirstIgnor,$tagW,$tagC,$vcfOut,$simDupPb,$chromDiv,$memNumReads,$chromSort,$minDepthSNV,$maxPval,$minDepthMeth,$maxStrandBias,$typeFasta,$multiSeqID,$peOverlap,$LastIgnor)=
&GetOptions($context,$checkBisulfite,$methNonCpGs,$qscore,$Q,$minSNVperc,$outDir,$bedOut,$wigOut,$p,$delDup,$FirstIgnor,$tagW,$tagC,$vcfOut,$simDupPb,$chromDiv,$memNumReads,$chromSort,$minDepthSNV,$maxPval,$minDepthMeth,$maxStrandBias,$typeFasta,$multiSeqID,$peOverlap,$LastIgnor);

##############   Starting MethylExtract  ############

print "\n################### Running MethylExtract v1.8.1 ###################\n\n";

print "\nFunctions resume:\n";
if ($typeFasta eq "Single") {print "Single-fasta references in $seqDir\n"}
else {print "Multi-fasta reference selected in $seqDir\n"}
...
if ($chromSort eq "N") {
	...
	my $returned=&StaticthreadsSplit($QueueSplit,$ResultsSplit,$inDir,\&splitChroms,$seqDir);
}
else {
	my %test=();
	if ($typeFasta eq "Single") {
		foreach (glob "$seqDir/*.fa") {
			$_=~/(\S+)\.fa$/i;
			my @splitFile=split(/\//,$1);
			$test{$splitFile[$#splitFile]}=1;
		}
		foreach (glob "$seqDir/*.fas") {
			$_=~/(\S+)\.fas$/i;
			my @splitFile=split(/\//,$1);
			$test{$splitFile[$#splitFile]}=1;
		}
		foreach (glob "$seqDir/*.fasta") {
			$_=~/(\S+)\.fasta$/i;
			my @splitFile=split(/\//,$1);
			$test{$splitFile[$#splitFile]}=1;
		}
	...
	foreach (glob "$inDir/*.sam") {
		$_=~/(\S+)\.sam$/i;
		my @splitFile=split(/\//,$1);
		if (!$test{$splitFile[$#splitFile]}) {die "Input files don't seem to be grouped by chromosome, please select chromSplitted=N\n"}
	}
	foreach (glob "$inDir/*.sam.gz") {
		$_=~/(\S+)\.sam.gz$/i;
		my @splitFile=split(/\//,$1);
		if (!$test{$splitFile[$#splitFile]}) {die "Input files don't seem to be grouped by chromosome, please select chromSplitted=N\n"}
	}
	foreach (glob "$inDir/*.bam") {
		$_=~/(\S+)\.bam$/i;
		my @splitFile=split(/\//,$1);
		if (!$test{$splitFile[$#splitFile]}) {die "Input files don't seem to be grouped by chromosome, please select chromSplitte=N\n"}
	}
}
...
my $returned=&Staticthreads($Queue,$Results,$seqDir,\&Main);
...
if ($context eq "CG" or $context eq "ALL") {
	...
	for (my $j=0;$j<=$#chroms;$j++) {
		if (-e "$outDir/CG_$chroms[$j].output") {
			...
			print "Warning: 'No CG methylation values on $chroms[$j]'";
			print "... probably there are no reads for this chromosome\n";
			print "... otherwise check strand FLAGs, Qscore type and minimum Qscore threshold chosen\n";
		}
...
if ($context eq "CHG" or $context eq "ALL") {
	...
	for (my $j=0;$j<=$#chroms;$j++) {
		if (-e "$outDir/CHG_$chroms[$j].output") {
			...
			print "Warning: 'No CHG methylation values on $chroms[$j]'";
			print "... probably there are no reads for this chromosome\n";
			print "... otherwise check strand FLAGs, Qscore type and minimum Qscore threshold chosen\n";
		}
...
if ($context eq "CHH" or $context eq "ALL") {
	...
	for (my $j=0;$j<=$#chroms;$j++) {
		if (-e "$outDir/CHH_$chroms[$j].output") {
			...
			print "Warning: 'No CHH methylation values on $chroms[$j]'";
			print "... probably there are no reads for this chromosome\n";
			print "... otherwise check strand FLAGs, Qscore type and minimum Qscore threshold chosen\n";
		}
...
if ($minSNVperc<=0.5) {
	...
	print VCF "##fileformat=VCFv4.1\n##source=MethylExtract 1.8.1\n##reference=folder:$seqDir\n##phasing=unphased\n";
...
if ($bedOut eq "Y") {
	if ($context eq "CG" or $context eq "ALL") {
		...
		print BEDOUT "track name=CG Methylation  description=MethylExtract_1.8.1 visibility=1 useScore=1\n";
		for (my $j=0;$j<=$#chroms;$j++) {
			if (-e "$outDir/CG_$chroms[$j].bed") {
	...
	if ($context eq "CHG" or $context eq "ALL") {
		...
		print BEDOUT "track name=CHG Methylation  description=MethylExtract_1.8.1 visibility=1 useScore=1\n";
		for (my $j=0;$j<=$#chroms;$j++) {
			if (-e "$outDir/CHG_$chroms[$j].bed") {
	...
	if ($context eq "CHH" or $context eq "ALL") {
		...
		print BEDOUT "track name=CHH Methylation  description=MethylExtract_1.8.1 visibility=1 useScore=1\n";
		for (my $j=0;$j<=$#chroms;$j++) {
			if (-e "$outDir/CHH_$chroms[$j].bed") {
...
if ($wigOut eq "Y") {
	if ($context eq "CG" or $context eq "ALL") {
		...
		print WIGOUT "track type=wiggle_0 name=CG Methylation  description=MethylExtract_1.8.1 glyph=wiggle_density bgcolor=red fgcolor=red\n";
		for (my $j=0;$j<=$#chroms;$j++) {
			if (-e "$outDir/CG_$chroms[$j].wig") {
	...
	if ($context eq "CHG" or $context eq "ALL") {
		...
		print WIGOUT "track type=wiggle_0 name=CHG Methylation  description=MethylExtract_1.8.1 glyph=wiggle_density bgcolor=red fgcolor=red\n";
		for (my $j=0;$j<=$#chroms;$j++) {
			if (-e "$outDir/CHG_$chroms[$j].wig") {
	...
	if ($context eq "CHH" or $context eq "ALL") {
		...
		print WIGOUT "track type=wiggle_0 name=CHH Methylation  description=MethylExtract_1.8.1 glyph=wiggle_density bgcolor=red fgcolor=red\n";
		for (my $j=0;$j<=$#chroms;$j++) {
			if (-e "$outDir/CHH_$chroms[$j].wig") {
...
print LOG "##########   MethylExtract_1.8.1 LogFile   ##########\n";
print LOG "\nWORKING FEATURES:\n";
print LOG "Input Directory: $inDir\n";
print LOG "$typeFasta-Fasta Sequences: $seqDir\n";
...
if (@ARGV) {
	...
	if($opts{inDir}){
		...
		if (-d $inDir) {
			...
			foreach (glob "$inDir/*.*") {
				if (-e $_) {
					$_ =~ m/(\w+)$/;
					if ($1 eq "sam" or $1 eq "gz" or $1 eq "bam") {$checkAlign="Y";}
					else {}
				}
				else {}
			}
			if ($checkAlign eq "N") {die "Don't exist *.bam, *.sam or *.gz files in the directory\n";}
			else {}
		}
	...
	if (defined($opts{FirstIgnor})) {
		if ($opts{FirstIgnor}>=1000) {$_[11]=0}
	...
	if($opts{seq}){
		...
		if (-d $seqDir) {
			...
			foreach (glob "$seqDir/*.fa $seqDir/*.fas $seqDir/*.fasta") {
				if (-e $_) {
					open testFA, "$_";
					my $line=<testFA>;
				...
				else {die "Doesn't exist *.fa, *.fas or *.fasta files on the directory\n";}
			}
			$_[23]="Single";
		}
		elsif (-e $seqDir) {
			...
			$_[24]=&GetMultiFastaID($seqDir);
			if ($#{$_[24]}>1) {$_[23]="Multi";}
	...
	print "  minDepthMeth=<minimum number of reads requiered to consider a methylation value in a certain position> [default: 3]\n  minDepthSNV=<minimum number of reads requiered to consider a SNV value in a certain position> [default: 1]\n";
...
return($inDir,$seqDir,$_[0],$_[1],$_[2],$_[3],$_[4],$_[5],$_[6],$_[7],$_[8],$_[9],$_[10],$_[11],$_[12],$_[13],$_[14],$_[15],$_[16],$_[17],$_[18],$_[19],$_[20],$_[21],$_[22],$_[23],$_[24],$_[25],$_[26]);
}
...
sub GetMultiFastaSeq {
	open (SEC, "$seqDir") or die "Multi-Fasta file doesn't exist\n";
	my $chromCheck=0;
	my $seqst_temp="";
	while (my $lineID=<SEC>) {
		if ($lineID=~/^>$_[0]/i) {
...
sub StaticthreadsSplit {
	...
	if ($typeFasta eq "Single") {
		...
		foreach (glob "$_[4]/*.fas") {
			$_=~/(\S+)\.fas$/i;
			my @splitFile=split(/\//,$1);
			$test{$splitFile[$#splitFile]}=1;
		}
		foreach (glob "$_[4]/*.fasta") {
			$_=~/(\S+)\.fasta$/i;
			my @splitFile=split(/\//,$1);
			$test{$splitFile[$#splitFile]}=1;
		}
	...
	foreach (glob "$_[2]/*.sam") {
		$_=~/(\S+)\.sam$/i;
		my @splitFile=split(/\//,$1);
		if ($test{$splitFile[$#splitFile]}) {die "$splitFile[$#splitFile].sam seems to be grouped by chromosome, please activate chromSplitted option and launch again\n"}
		$_[0]->enqueue("$_");
	}
	foreach (glob "$_[2]/*.sam.gz") {
		$_=~/(\S+)\.sam.gz$/i;
		my @splitFile=split(/\//,$1);
		if ($test{$splitFile[$#splitFile]}) {die "$splitFile[$#splitFile].sam.gz seems to be grouped by chromosome, please activate chromSplitted option and launch again\n"}
		$_[0]->enqueue("$_");
	}
	foreach (glob "$_[2]/*.bam") {
		$_=~/(\S+)\.bam$/i;
		my @splitFile=split(/\//,$1);
		if ($test{$splitFile[$#splitFile]}) {die "$splitFile[$#splitFile].bam seems to be grouped by chromosome, please activate chromSplitted option and launch again\n"}
		$_[0]->enqueue("$_");
	}
	for (1..$p) {
		$_[0]->enqueue(undef);
...
sub Staticthreads {
	my $returnData=\my @returnData;
	if ($typeFasta eq "Single") {
		foreach (glob "$_[2]/*.fa $_[2]/*.fas $_[2]/*.fasta") {$_[0]->enqueue("$_");}
	}
	elsif ($typeFasta eq "Multi") {
